...
coef_values <- coef(lasso_model, s = "lambda.min") |>
  as.matrix() |>
  as.vector()
...
predictors <- c("D_pv2p_lag1", "D_pv2p_lag2", "latest_pollav_DEM", 
                "mean_pollav_DEM", "CPI", "GDP_growth_quarterly", 
                "log(contribution_receipt_amount)")

text_form <- paste0(
  "y = ", round(intercept, 4), " + ",
  paste(
    paste0(round(coefs, 4), " * ", predictors),
    collapse = " + "
  )
)
```
